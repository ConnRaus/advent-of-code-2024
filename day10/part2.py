...
def recursePaths(heightMap, pos, prevHeight=-1, visited=None):
    if visited is None:
        visited = set()
    ...
    if pos in visited or currentHeight-prevHeight != 1:
        return 0
    if currentHeight == 9:
        return 1
    ...
    for dx, dy in directions:
        totalPaths+= recursePaths(heightMap, (currentX+dx, currentY+dy), currentHeight, visited)
