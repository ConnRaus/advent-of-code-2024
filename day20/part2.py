...
def generateShortcuts(distance=20):
    shortcuts = []
    for x in range(-distance, distance + 1):
        max_y = distance - abs(x)
...
def checkShortcuts(pos, distanceDict):
    ...
    directions = generateShortcuts(20) # check all possible shortcut jumps
    ...
    for i,(dx,dy) in enumerate(directions):
        ...
        if isInBounds((nx, ny), maze):
            if (nx, ny)  in distanceDict.keys():
                savings[i] = (distanceDict[(nx,ny)] - distanceDict[pos]) - (abs(dx)+abs(dy)) # subtract distance jumped
    return savings
