...
def parseInput(filename):
    ...
    for i,line in enumerate(fileinput.input(filename)):
        ...
        if lineType == 0 or lineType == 1:
            ...
            num1 = int(line[0][line[0].index('=')+1:]) + 10000000000000
            num2 = int(line[1][line[1].index('=')+1:]) + 10000000000000
        else:
...
for numList in nums:
    ...
    threshold = 0.0000001
    if abs(pushA-int(pushA)) > threshold or abs(pushB-int(pushB)) > threshold: 
